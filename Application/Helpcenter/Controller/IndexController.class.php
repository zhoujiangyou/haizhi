...
namespace Helpcenter\Controller;
...
class IndexController extends Controller {
    ...
    function _initialize() {
        ...
        $catTitle = modC('PAPER_CATEGORY_TITLE', L('_MODULE_'), 'Helpcenter');
    ...
    public function index() {
        $model = D('HelpcenterCategory');
        $catList = $model->categoryList(array('status' => 1));
        if (count($catList)) {
        ...
        $map['status'] = 1;
        $pageArtiles = $this->helpCenterModel->getList($map, 'id,title,sort,category');
        ...
        $data = $this->helpCenterModel->getData($aId);
