...
class ConfigController extends AdminController
{
    ...
    public function website()
    {
        ...
        $builder->keySingleImage('JUMP_BACKGROUND', L('_IMG_BG_REDIRECTED_'));
    ...
    public function smsManage($page = 1, $r = 10){
        $builder = new AdminListBuilder();
        $model = M('action_message');
        $list = $model->page($page, $r)->select();
        $totalCount = $model->count();
        $builder->title('短信平台配置')
            ->buttonNew(U('addSms'),'新增配置')
            ->ajaxButton(U('delSms'),'','删除配置')
            ->keyId()
            ->key('action_name','行为名称','label')
            ->keyYesNo('is_sms','是否发送短信')
            ->keyYesNo('is_message','是否发送站内信')
            ->keyYesNo('is_email','是否发送邮件')
    ...
    public function addSms(){
        if(IS_POST){
            $data=I('post.');
            $model = M('action_message');
            empty($data)? : $this->ajaxReturn(array('info'=>'数据为空','status'=>0));
            if($model->add($data)){
                  $this->ajaxReturn(array('info'=>'新增成功','status'=>1,'url'=>U('smsmanage')));
            }else{
          ...
          $builder = new AdminConfigBuilder();
          $builder->title('新增配置');
          $builder->keyText('action_name','操作名称','请管理员填写操作action完整路径 如：admin/project/add');
          $builder->keyBool('is_sms','是否发送短信')->keyBool('is_message','是否发送站内信')->keyBool('is_email','是否发送邮件');
          $builder->keyEditor('sms_template','短信模板','','all',array('width'=>'800px','height'=>'400px'));
          $builder->keyEditor('message_template','站内信模板','','all',array('width'=>'800px','height'=>'400px'));
          $builder->keyEditor('email_template','邮件模板','','all',array('width'=>'800px','height'=>'400px'));
          $builder->buttonSubmit();
          $builder->display();
        }
    }
