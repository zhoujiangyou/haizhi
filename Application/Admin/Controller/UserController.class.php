...
class UserController extends AdminController
{
    ...
    public function changeGroup()
    {
        ...
        if ($_POST['do'] == 1) {
            ...
            $this->success(L('_SUCCESS_'),U('user/index'));
        } else {
...
/******************个人认证*******************************/
    ...
    public function vertify(){
        ...
        if(!empty($data)){
            foreach($data as $key=>$value){
                $data[$key]['status_text'] = $value['is_pass'] ==1?'认证通过':($value['is_pass'] ==0?'待认证':'认证失败');
            }
        }
        $this->assign('_list',$data);
        $this->assign('_page',$show);// 赋值分页输
        $this->display();
    }
    ...
    public function doVertify(){
        $id = I('post.id');
        $data = array('is_pass'=>1);

        if(M('personal_vertify')->where($id)->getField('is_pass')==1){
            $this->success('认证成功');
        }

        if($id){
            if(M('personal_vertify')->where('id='.$id)->save($data)){
                $this->ajaxReturn(array('info'=>'认证成功','status'=>1,'url'=>U('vertify')));
            }
        }else{
            $this->ajaxReturn(array('info'=>'认证失败','status'=>0,'url'=>U('vertify')));
        }
    }

    public function dounvertify(){
        $id = I('post.id');
        $data = array('is_pass'=>-1);
        if(M('personal_vertify')->where('id='.$id)->getField('is_pass')==-1) {
            $this->success('操作成功');
        }
        if($id){
            if(M('personal_vertify')->where('id='.$id)->save($data)){
    ...
    public function deleteVertify(){

        $ids = I('post.id');
        if(!empty($ids)){
            foreach($ids as $key=>$value){
                M('personal_vertify')->delete($value);
            }
            $this->success('删除成功');
        }
        $this->error('删除失败');
...
/******************组织机构认证*******************************/
    ...
    public function vertifyB(){
        $model = D('Vertify');
        $count = $model->sumB();
        $Page       = new \Think\Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数
        $show       = $Page->show();// 分页显示输出
        $data = $model->listsB($Page);
        ...
        $this->assign('_list',$data);
        $this->assign('_page',$show);// 赋值分页输
        $this->display('vertifyg');
    }

    public function showDetailB(){
        $id = I('id');
        $model = D('Vertify');
        $data= $model->getB($id);
        $this->assign('data',$data);
        $this->display('detailg');
    }

    public function doVertifyB(){
        $id = I('post.id');
        $data = array('is_pass'=>1);

        if(M('business_vertify')->where('id='.$id)->getField('is_pass')==1){
            $this->ajaxReturn(array('info'=>'认证成功','status'=>1,'url'=>U('vertifyB')));
        }

        if($id==0 ||$id!=null){
            if(M('business_vertify')->where('id='.$id)->save($data)){
                $this->ajaxReturn(array('info'=>'认证成功','status'=>1,'url'=>U('vertifyB')));
            }
        }else{
            $this->ajaxReturn(array('info'=>'认证失败','status'=>0,'url'=>U('vertifyB')));
        }
    }

    public function dounvertifyB(){
        $id = I('post.id');
        $data = array('is_pass'=>-1);
        if(M('business_vertify')->where('id='.$id)->getField('is_pass')==-1) {
            $this->success('操作成功');
        ...
        if($id==0||$id!=null){
            if(M('business_vertify')->where('id='.$id)->save($data)){
                $this->ajaxReturn(array('info'=>'操作成功','status'=>1,'url'=>U('vertifyB')));
            }
        }else{
            $this->ajaxReturn(array('info'=>'操作失败','status'=>0,'url'=>U('vertifyB')));
        }
    }

    public function addVertifyB(){
        $data = I('post.');
    }


    public function deleteVertifyB(){
       $ids = I('post.id');
        if(!empty($ids)){
         foreach($ids as $key=>$value){
           M('business_vertify')->delete($value);
         }
            $this->ajaxReturn(array('info'=>'删除成功','status'=>1,'url'=>U('vertifyB')));
        }
        $this->ajaxReturn(array('info'=>'删除失败','status'=>0,'url'=>U('vertifyB')));
    }

/******************专家认证*******************************/
    public function vertifyE(){
        $model = D('Vertify');
        $count = $model->sumE();
        $Page       = new \Think\Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数
        $show       = $Page->show();// 分页显示输出
        $data = $model->listsE($Page);

        if(!empty($data)){
            foreach($data as $key=>$value){
                $data[$key]['status_text'] = $value['is_pass'] == 1?'认证通过':($value['is_pass'] ==0?'带认证':'认证失败');
            }
        }
        $this->assign('_list',$data);
        $this->assign('_page',$show);// 赋值分页输
        $this->display('vertifye');
    }
    ...
    public function showDetailE(){
        ...
        $data= $model->getE($id);
        $this->assign('data',$data);
        $this->display('detaile');
    }

    public function doVertifyE(){
        $id = I('post.id');
        $data = array('is_pass'=>1);

        if(M('expert_vertify')->where('id='.$id)->getField('is_pass')==1){
            $this->success('认证成功');
        }

        if($id){
            if(M('expert_vertify')->where('id='.$id)->save($data)){
                $this->success('认证成功');
            }
        }else{
            $this->error('认证失败');
    ...
    public function addVertifyE(){
        $data = I('post.');
    }


    public function deleteVertifyE(){

        $ids = I('post.id');
        if(!empty($ids)){
            foreach($ids as $key=>$value){
                M('expert_vertify')->delete($value);
            }
            $this->success('删除成功');
        }
        $this->error('删除失败');
