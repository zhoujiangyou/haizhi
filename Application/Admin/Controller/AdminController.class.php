...
class AdminController extends Controller
{
    ...
    /**
     * 检查action访问，是否在配置表中。如果在则执行信息发送，不在则跳过
     *
     * void
     *
     * @author 周海天  <xcoolcc@gmail.com>
     */
   final  protected function accessAction(){
    ...
    final protected  function comeFromProject($authdata){
        if(IS_POST){
            ...
            $data = array(
                ...
                'sms_template'=>$authdata['sms_template'],
                'message_template'=>$authdata['message_template'],
                'email_template'=>$authdata['email_template'],
                'uid'=>$uid,
            );
            return $data;
        }else{
    ...
    //todo
    //判断是否是是从文章过来的
    final protected  function comeFromArticle($authdata){
        $uid = is_login();
        $articleName = I('post.title');
        if(empty($projectName)){
           $articleName = M('news')->where(I('id'))->getField('title');
        }
        $user = M('ucenter_member')->where('id=%d',$uid)->find();
        $data = array(
            'username'=>$user['username'],
